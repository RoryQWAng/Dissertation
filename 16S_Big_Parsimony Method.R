...
BS_16S <- na.omit(select(big_seq, "Sample", "16S", "Encrustment"))
BS_16S <- select(BS_16S, "Sample", "16S") 
BS16Sl_names <- BS_16S$"Sample"
...
BS16Sl_dist <- seqinr::dist.alignment(BS16Sl_align_seqinr_ID, 
                                      matrix = "identity") #Replace this with the trait distance matrix, make empty nxn matrix, for each sample, you compare each trait for each trait looked at to create a distance matrix, 
for (i in 1:n){
  for (j in 1:n){
    if (df$trait[i]!=df$trait[j]){
      distanceMatrix[i][j]!=traitWeight
    }}}
...
BS_16S_names_shuffled <- c(length(1:length(BS_16S_ID)))
for(i in 1:length(BS16Sl_names)){
  BS_16S_names_shuffled[i] <- BS16Sl_names[keyvector[i]]
...
plot.phylo(BS16Sl_tree, main="COI Phylogram", 
           type = "phylogram", 
           label.offset = 3,
           use.edge.length = F)
...
crust_mode_shuffled <- c(1:length(BS16Sl_names))
for(i in 1:length(BS16Sl_names)){
...
fitER
plot.phylo(absolute_tree, type="phylogram", label.offset = 0.03, cex = 0.4, use.edge.length = F)
tiplabels(pie=to.matrix(crust_mode, levels(crust_mode)),piecol=cols,cex=0.1)
