...
BS_16S <- na.omit(select(big_seq, "Sample", "16S", "Skeleton"))
...
Skeleton_mode_og <- read.csv("Data/zo_character.csv", row.names=NULL) %>% dplyr::rename("16S" = X16S,
                                                                                     "18S" = X18S,
                                                                                     "ITS" = ITS.rDNA) 

Skeleton_mode <- Skeleton_mode_og[!is.na(Skeleton_mode_og$"16S"),]
#crust_mode
#Skeleton_mode$"Skeleton"[is.na(Skeleton_mode$"Skeleton")] <- "Unknown"
Skeleton_mode <- Skeleton_mode[!is.na(Skeleton_mode$"Skeleton"),] 

#good_tip <- which(crust_mode$"Encrustment" != "Unknown")
Skeleton_mode <- Skeleton_mode %>% filter(Skeleton != "Unknown")
#crust_mode
Skeleton_mode <- setNames(Skeleton_mode[,10],rownames(Skeleton_mode))

Skeleton_mode_shuffled <- c(1:length(Skeleton_mode))
for(i in 1:length(BS_16S_names)){
  Skeleton_mode_shuffled[i] <- Skeleton_mode[keyvector[i]]
  
}
cbind(Skeleton_mode_shuffled, BS_16S_names_shuffled)

Skeleton_mode <- as.factor(Skeleton_mode_shuffled)

plotTree(BS16Sl_tree,type="fan",fsize=0.8,ftype="i",lwd=2)
#length(levels(crust_mode))
#levels(crust_mode)
cols <- setNames(mycols,levels(Skeleton_mode))[c(1:length(levels(Skeleton_mode)))]
tiplabels(pie=to.matrix(Skeleton_mode,
                        levels(Skeleton_mode)),piecol=cols,cex=0.2)
to.matrix(Skeleton_mode,
          levels(Skeleton_mode))
...
fitER<-ace(Skeleton_mode,absolute_tree,model="ER",type="discrete")
fitER
plot.phylo(absolute_tree, type="phylogram", label.offset = 0.03, cex = 0.5)
tiplabels(pie=to.matrix(Skeleton_mode, levels(Skeleton_mode)),piecol=cols,cex=0.1)
