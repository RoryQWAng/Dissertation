...
big_seq <- read.csv("Data/Zoa_character.csv", header = T) %>% dplyr::rename("16S" = X16S,
                                                                            "18S" = X18S,
                                                                            "ITS" = ITS.rDNA) #Fetch and clean zoanthid file
big_seq <- select(big_seq, "Sample", "Canals", "Zooxanthellate", "Gender", "Encrustment","Skeleton", "COI", "16S", "18S", "ITS") # select relevant types
BS_18S <- na.omit(select(big_seq, "Sample", "18S", "Encrustment")) # Select information you care about and remove all NA rows
BS_18S <- select(BS_18S, "Sample", "18S") #Remove Encrustment
...
crust_mode_shuffled <- c(1:24)
for(i in 1:length(BS_18S_names)){
  crust_mode_shuffled[i] <- crust_mode[keyvector[i]]
  
}
cbind(crust_mode_shuffled, BS_18S_names_shuffled)
...
fitER
plot.phylo(absolute_tree, type="phylogram", label.offset = 0.03)
...
nodelabels(node=1:absolute_tree$Nnode+Ntip(absolute_tree),
           pie=fitER$lik.anc,piecol=cols,cex=0.4, label.offset = 3)
